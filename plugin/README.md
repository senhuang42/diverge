...
`ready`, `generating`, `results`, `brief-results`, `marked`, `recent`, `settings`, or `error`
before opening the Standalone build. The `marked` fixture renders a part-judged sheet so the
keep, pass, favourite, and exported grease-pencil marks can be reviewed. The experimental `map` fixture remains available for comparative testing. Set
